...
The public page, decided 2026-09-15: this repo stays the schema's stable home and hosts nothing itself, and the method's public address is a takeaway on the show's site at `https://sigsub.show/takeaways/name-the-human/`. That address and the takeaway noun are registered in the show repo's `ARTIFACTS.md`; the address is settled but the page is not live until the Sunday send after Ep 12, and the collection builder that will render it does not exist yet. This is not a reversal of the no-docs-site decision above. The repo still ships no hosted surface of its own; it points at one that belongs to another scope, which is also why the schema `$id` resolves to a tag on this repository rather than to any website.
...
- 2026-09-15: Repository created from the Ep 12 handoff. Schema v0.3, the five scripts, fixture estate, synthetic examples, SKILL.md, prompt, README with the privacy statement and prior art. Version 0.1.0. Decisions recorded above: MIT, `skill-a11y-audit` layout, no distribution surfaces, real inventory never committed, method registration deferred to the two-instances rule.
...
- 2026-09-15: Public address settled. The method's page will be a takeaway at `https://sigsub.show/takeaways/name-the-human/`, registered in the show repo's `ARTIFACTS.md` along with the takeaway noun. Nothing here claims the page is live until it ships: `homepageUrl` still points at the show root, the README's two promises to link Ep 12 and the page stay unfilled, and `imgs/og.svg` carries no domain line.
